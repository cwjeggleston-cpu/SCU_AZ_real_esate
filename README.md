...
- **DPT cohort overlap (5,500 SF)** — DPT runs two cohorts' labs simultaneously, one week × three times per year. Each scenario resolves it one of three ways: a per-scenario toggle, ON by default = rent surge space (e.g., a conference center — shown as a dashed plate, separate from permanent GSF); OFF = absorb it into a chosen permanent site (hatched block, counts against that site's capacity).
