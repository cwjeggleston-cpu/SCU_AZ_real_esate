...
const STORAGE_KEY = "scu-phx-planner-v2";
...
const defaultData = () => ({
  ...
  sites: [
    ...
    { id: "scottsdale", name: "Scottsdale — Candidate", capacity: 20000, rate: 45, ti: 0,
      note: "≈20,000 SF candidate site @ $45/SF/yr." },
    { id: "surge", name: "Surge Site", capacity: 5500, rate: 0, ti: 0, timeShared: true,
      note: "Rented one week, three times per year (lab intensives). Programs rotate through the same flat SF — no ×multiplier." },
  ...
  scenarios: [
    {
      ...
      note: "Every program at Sonoran; OTD & SLP intensives at the surge site.",
      sharing: false, includeAnatomy: true, useSurge: true,
      assign: { dc: "sonoran", dpt: "sonoran", otd: "surge", slp: "surge", pa: "sonoran", gc: "sonoran", anat: "sonoran" },
    },
    {
      id: "s2", name: "2 · SPLIT Campus: PA/GC separate",
      note: "PA & GC at Scottsdale; all other programs remain at Sonoran.",
      sharing: false, includeAnatomy: true, useSurge: true,
      assign: { dc: "sonoran", dpt: "sonoran", otd: "surge", slp: "surge", pa: "scottsdale", gc: "scottsdale", anat: "sonoran" },
    },
    {
      id: "s3", name: "3 · SPLIT Campus: DC/PA off campus",
      note: "DPT, OTD, SLP, GC (and Anatomy) at Sonoran; DC & PA at Scottsdale.",
      sharing: false, includeAnatomy: true, useSurge: true,
      assign: { dc: "scottsdale", dpt: "sonoran", otd: "surge", slp: "surge", pa: "scottsdale", gc: "sonoran", anat: "sonoran" },
    },
    {
      id: "s4", name: "4 · Sublease Sonoran",
      note: "All programs relocate off-site; Sonoran space is subleased.",
      sharing: false, includeAnatomy: true, useSurge: true,
      assign: { dc: "scottsdale", dpt: "scottsdale", otd: "surge", slp: "surge", pa: "scottsdale", gc: "scottsdale", anat: "scottsdale" },
    },
  ],
  activeScenarioId: "s1",
});
...
function computeScenario(scn, data) {
  ...
  const useSurge = scn.useSurge !== false; // default on, incl. older saved scenarios

  const groups = {};
  included.forEach((p) => {
    let sid = bySite[scn.assign[p.id]] ? scn.assign[p.id] : "unassigned";
    if (!useSurge && bySite[sid] && bySite[sid].site.timeShared) sid = "unassigned";
  ...
  Object.entries(groups).forEach(([sid, progs]) => {
    ...
    if (bucket.site.timeShared) {
      progs.forEach((p) => bucket.blocks.push({ program: p, gsf: programNSF(p), kind: "flat" }));
      bucket.used = Math.max(0, ...bucket.blocks.map((b) => b.gsf));
      return;
    }
    ...
    sharers.forEach((p) => {
      ...
      sumShared += S;
      // dedicated remainder: overhead on the shared slice + full GSF on the unshared slice
  ...
  const displaced = !useSurge
    ? included.filter((p) => { const b = bySite[scn.assign[p.id]]; return b && b.site.timeShared; })
    : [];

  return { bySite, total, overs, paAtRisk, shareSavings, annualCost, tiCost, useSurge, displaced };
}
...
const SURGE_INFO = "Surge space is driven by DPT running two cohorts' labs at the same time — rented one week, three times per year. Turn it off if a permanent site is large enough to absorb the need (e.g., taking a whole floor instead of renting surge space).";
  ...
  return (
    <div style={{ display: "flex", gap: 18, alignItems: "flex-end", flexWrap: "wrap" }}>
      {order.map((sid) => {
        ...
        if (b.site.timeShared && result.useSurge === false) return null;
          ...
          <div key={sid} style={{ width: 150, flexShrink: 0 }}>
            <div style={{ position: "relative", height: H, borderBottom: `3px solid ${BRAND.ink}` }}>
              ...
              <div style={{
                ...
                {b.blocks.filter((x) => x.gsf > 0).map((x, i, arr) => {
                  const drawSF = b.site.timeShared ? b.used / arr.length : x.gsf;
                  const h = Math.max((drawSF / maxRef) * H, 4);
                  const isPool = x.kind === "pool";
                  return (
                    <div
                      key={i}
                      title={isPool ? `${x.program.name}: ${fmt(x.gsf)} SF counted once` : `${x.program.code}: ${fmt(x.gsf)} ${b.site.timeShared ? "SF (time-shared)" : "GSF"}`}
                      style={{
                        height: h,
                        background: isPool
                          ? `repeating-linear-gradient(45deg, ${BRAND.blue}, ${BRAND.blue} 6px, ${PROGRAM_COLORS.shared} 6px, ${PROGRAM_COLORS.shared} 12px)`
                    ...
                    >
                      {h >= 15 ? (isPool ? `SHARED ${fmt(x.gsf)}` : `${x.program.code} ${fmt(x.gsf)}`) : ""}
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{ marginTop: 8 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: BRAND.ink, fontFamily: "'Montserrat', sans-serif", display: "flex", alignItems: "center", gap: 5 }}>
                {b.site.name}{b.site.timeShared && <Info text={SURGE_INFO} />}
              </div>
              <div style={{ fontSize: 12, color: over ? BRAND.red : BRAND.slate, fontWeight: over ? 700 : 500, fontVariantNumeric: "tabular-nums" }}>
                {fmt(b.used)} {b.site.timeShared ? "SF" : "GSF"}{pct !== null ? ` · ${pct}% of cap` : ""}
                {over ? ` · over by ${fmt(b.used - cap)}` : ""}
              </div>
              {b.site.timeShared && (
                <div style={{ fontSize: 10, color: BRAND.slate, fontStyle: "italic" }}>time-shared rotation · flat SF, no multiplier</div>
              )}
            </div>
          </div>
        );
    ...
    <div style={{ minHeight: "100vh", background: BRAND.mist, fontFamily: "'Public Sans', sans-serif", color: BRAND.ink }}>
      ...
      <nav style={{ background: "#fff", borderBottom: `1px solid ${BRAND.line}`, padding: "0 28px", display: "flex", gap: 6, overflowX: "auto" }}>
        ...
        <button onClick={resetAll} style={{
          marginLeft: "auto", alignSelf: "center", background: "none", border: "none",
          color: BRAND.slate, fontSize: 11.5, cursor: "pointer", whiteSpace: "nowrap", padding: "8px 4px",
          ...
          <>
            ...
            {result.displaced.length > 0 && (
              <div style={{ ...card, borderLeft: `5px solid ${BRAND.gold}`, marginBottom: 14, padding: "14px 20px", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                <Tag color="#fff" bg={BRAND.gold}>Needs a Home</Tag>
                <span style={{ fontSize: 13 }}>
                  Surge space is off, so <b>{result.displaced.map((p) => p.code).join(", ")}</b> now {result.displaced.length === 1 ? "needs" : "need"} permanent space at full GSF — currently counted as Unassigned. Reassign below (tip: turn on shared core to co-locate efficiently).
                </span>
              </div>
            )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "minmax(340px, 5fr) minmax(380px, 7fr)", gap: 18, alignItems: "start" }}>
              ...
              <section style={card}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: "12px 12px", background: BRAND.mist, borderRadius: 8, marginBottom: 12 }}>
                  <Toggle
                    checked={scn.useSurge !== false}
                    onChange={(v) => updateScn((s) => (s.useSurge = v))}
                    label={<span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>Use surge space (5,500 SF rental) <Info text={SURGE_INFO} /></span>}
                  >
                    {scn.useSurge === false && <Tag>off — absorb into a permanent site</Tag>}
                  </Toggle>
                  <Toggle
                    checked={!!scn.sharing}
                ...
                {data.programs.map((p) => {
                  ...
                  const surgeOn = scn.useSurge !== false;
                  const rawSite = data.sites.find((s) => s.id === scn.assign[p.id]);
                  const effSiteId = rawSite && rawSite.timeShared && !surgeOn ? "unassigned" : (scn.assign[p.id] || "unassigned");
                  const assignedSite = data.sites.find((s) => s.id === effSiteId);
                  const onSurge = assignedSite && assignedSite.timeShared;
                  const gsf = onSurge ? programNSF(p) : programGSF(p, data.multiplier);
                  const sharesHere = scn.sharing && (p.sharePct || 0) > 0 && p.id !== "anat" && !onSurge;
                  return (
                    <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 0", borderBottom: `1px solid ${BRAND.mist}`, opacity: isAnatOff ? 0.4 : 1 }}>
                      ...
                      <div style={{ textAlign: "right", fontVariantNumeric: "tabular-nums", fontSize: 12.5, fontWeight: 600, minWidth: 92 }}>
                        {isAnatOff ? "—" : fmt(gsf)}{" "}
                        <span style={{ color: BRAND.slate, fontWeight: 400 }}>{onSurge ? "SF flat" : "GSF"}</span>
                      </div>
                      <select
                        value={effSiteId}
                        disabled={isAnatOff}
                      ...
                      >
                        {data.sites.filter((s) => surgeOn || !s.timeShared).map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                ...
                <p style={{ fontSize: 11, color: BRAND.slate, marginTop: 12, lineHeight: 1.5 }}>
                  Programs on the Surge Site count at flat lab SF — it's rented turnkey and time-shared, so the ×{data.multiplier} multiplier doesn't apply there. Set each program's shareable % on the Program Space Model tab.
                </p>
              </section>
              ...
              <section style={card}>
                ...
                <div style={{ marginTop: 22, borderTop: `2px solid ${BRAND.line}`, paddingTop: 14, display: "flex", flexWrap: "wrap", gap: 26 }}>
                  <div>
                    <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: BRAND.slate }}>Total footprint</div>
                ...
                <p style={{ fontSize: 11, color: BRAND.slate, marginTop: 12, lineHeight: 1.5 }}>
                  The striped SHARED block is the pooled core lab, counted once per site (largest sharer's need). Each sharing program keeps its dedicated remainder — offices, storage, circulation from the multiplier — as its own block. Without sharing, treat totals as the not-to-exceed number.
                </p>
              </section>
            </div>
          </>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ borderCollapse: "collapse", width: "100%", fontSize: 12.5, fontVariantNumeric: "tabular-nums" }}>
                ...
                <tbody>
                  ...
                  <tr>
                    <td style={{ padding: "10px", fontWeight: 800, fontFamily: "'Montserrat', sans-serif" }}>Total GSF</td>
                    {data.scenarios.map((s) => (
                  ...
                  <tr>
                    <td style={{ padding: "8px 10px", color: BRAND.slate }}>Surge space</td>
                    {data.scenarios.map((s) => (
                      <td key={s.id} style={{ padding: "8px 10px", textAlign: "right", color: BRAND.slate }}>{s.useSurge === false ? "Off" : "In use"}</td>
                    ))}
                  </tr>
                  <tr>
      ...
      <footer style={{ padding: "16px 28px", fontSize: 11, color: BRAND.slate, borderTop: `1px solid ${BRAND.line}`, background: "#fff" }}>
        Southern California University of Health Sciences · Facilities scenario model · Figures update live and save automatically. NSF = net program space; GSF applies the ×{data.multiplier} multiplier; Surge Site counts flat SF.
      </footer>
    </div>
  );
}
