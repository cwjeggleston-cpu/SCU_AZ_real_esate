...
function computeScenario(scn, data) {
  ...
  const overlap = scn.overlap || { mode: "surge", siteId: "scottsdale" };
  ...
  let surgeSF = 0;
  if (overlap.mode === "surge") {
    surgeSF = overlapSF;
  } else if (overlapSF > 0) {
    const host = bySite[overlap.siteId] || bySite.scottsdale || bySite.unassigned;
    host.blocks.push({
  ...
  return { bySite, total, overs, paAtRisk, shareSavings, annualCost, tiCost, overlap, overlapSF, surgeSF };
}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "minmax(340px, 5fr) minmax(380px, 7fr)", gap: 18, alignItems: "start" }}>
              ...
              <section style={card}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: "12px 12px", background: BRAND.mist, borderRadius: 8, marginBottom: 12 }}>
                  {/* DPT cohort overlap: surge rental toggle */}
                  <Toggle
                    checked={(scn.overlap || { mode: "surge" }).mode === "surge"}
                    onChange={(v) => updateScn((sc) => (sc.overlap = { mode: v ? "surge" : "absorb", siteId: (sc.overlap && sc.overlap.siteId) || "scottsdale" }))}
                    label={<span style={{ display: "inline-flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>Surge space rental · <NumInput value={data.overlapSF} step={500} width={70} onChange={(v) => update((d) => (d.overlapSF = v))} /> SF (1 wk × 3/yr) <Info text={SURGE_INFO} /></span>}
                  >
                    {(scn.overlap || { mode: "surge" }).mode === "surge" ? (
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 4, color: BRAND.slate, fontSize: 11.5 }}>
                        est. <NumInput value={data.surgeRentAnnual} step={1000} width={82} onChange={(v) => update((d) => (d.surgeRentAnnual = v))} /> $/yr
                      </span>
                    ) : (
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 11.5, color: BRAND.slate }}>
                        off — absorbed at
                        <select
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    {data.scenarios.map((s) => {
                      const r = computeScenario(s, data);
                      const m = (r.overlap || {}).mode;
                      const siteName = m !== "surge" ? ((data.sites.find((x) => x.id === (r.overlap || {}).siteId) || {}).name || "Scottsdale") : "";
                      return (
                        <td key={s.id} style={{ padding: "8px 10px", textAlign: "right" }}>
                          {m === "surge" ? <span style={{ color: BRAND.slate }}>Rented</span>
                            : <span style={{ color: BRAND.slate }}>Absorbed · {siteName.split(" — ")[0] || "Scottsdale"}</span>}
                        </td>
                      );
                    })}
                  </tr>
